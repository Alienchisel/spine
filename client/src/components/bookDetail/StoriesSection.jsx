        ...
        <div className="space-y-2 mb-3">
          {visibleStories.map(s => editId === s.id ? (
            ...
            <div key={s.id} className="flex items-center gap-2 group text-xs">
              ...
              <button type="button" onClick={() => startEdit(s)} aria-label={`Edit ${noun} ${s.title}`} className="text-neutral-700 hover:text-neutral-400 opacity-30 group-hover:opacity-100 group-focus-within:opacity-100 transition-all flex-shrink-0">Edit</button>
              <button type="button" onClick={() => handleDelete(s.id, s.title)} title={`Delete ${noun}`} aria-label={`Delete ${noun} ${s.title}`} className="text-neutral-700 hover:text-warn opacity-30 group-hover:opacity-100 group-focus-within:opacity-100 transition-all flex-shrink-0">×</button>
            </div>
          ))}
        </div>
      ) : (
