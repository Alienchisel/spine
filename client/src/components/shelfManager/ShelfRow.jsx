  ...
  return (
    <div className="flex items-center justify-between py-1.5 pl-16 pr-2 group">
      ...
      <div className="flex items-center gap-2 opacity-30 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
        <button type="button" onClick={() => setEditing(true)} title="Rename shelf" aria-label="Rename shelf" className="text-xs text-neutral-600 hover:text-neutral-300 transition-colors">✎</button>
        <button type="button" onClick={() => onDelete(shelf.id)} title="Delete shelf" aria-label="Delete shelf" className="text-xs text-neutral-600 hover:text-warn transition-colors">×</button>
      </div>
    </div>
  );
}
