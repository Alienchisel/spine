...
describe('stats', () => {
  ...
  describe('GET /api/stats counts reflect data', () => {
    ...
    // avgDaysToFinish is a whole-corpus AVG, so it can't be pinned to an exact
    // value on this shared DB (it averages over every other test's finished
    // reads). Its exact-arithmetic check lives in test/stats-avg-days.test.js,
    // which gets its own process (and thus its own DB) — the only real
    // isolation boundary, since a second createTestServer in the same file
    // shares the module-cached connection.
