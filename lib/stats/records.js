...
export function getRecords() {
  return {
    ...
    // "Has been read at least once" — under Phase 3 that's exactly
    // "status = 'finished'" (every finished book has at least one
    // reads row thanks to the cascade and migrations 076/078).
    longestReadPhysical:   bookRecord(`SELECT * FROM books WHERE status = 'finished' AND format = 'physical'  AND page_count > 0       ORDER BY page_count DESC LIMIT 1`),
    shortestReadPhysical:  bookRecord(`SELECT * FROM books WHERE status = 'finished' AND format = 'physical'  AND page_count > 0       ORDER BY page_count ASC  LIMIT 1`),
    longestReadDigital:    bookRecord(`SELECT * FROM books WHERE status = 'finished' AND format = 'ebook'     AND page_count > 0       ORDER BY page_count DESC LIMIT 1`),
    shortestReadDigital:   bookRecord(`SELECT * FROM books WHERE status = 'finished' AND format = 'ebook'     AND page_count > 0       ORDER BY page_count ASC  LIMIT 1`),
    longestReadAudiobook:  bookRecord(`SELECT * FROM books WHERE status = 'finished' AND format = 'audiobook' AND duration_minutes > 0 ORDER BY duration_minutes DESC LIMIT 1`),
    shortestReadAudiobook: bookRecord(`SELECT * FROM books WHERE status = 'finished' AND format = 'audiobook' AND duration_minutes > 0 ORDER BY duration_minutes ASC  LIMIT 1`),
    ...
    // earliest / most-recent finish across all books. Phase 3 simplified
    // the COALESCE-with-book-column fallback the bridge needed; reads
    // is now the single source. dateFinishedFrom points bookRecord at
    // the aliased column so the response surfaces the effective date.
    firstFinished: bookRecord(`
      SELECT b.*,
        (SELECT MIN(date_finished) FROM reads WHERE reads.book_id = b.id) AS effective_date_finished
      FROM books b
      WHERE EXISTS (SELECT 1 FROM reads WHERE reads.book_id = b.id AND date_finished IS NOT NULL)
      ORDER BY effective_date_finished ASC
      LIMIT 1
    `, { dateFinishedFrom: 'effective_date_finished' }),
    lastFinished: bookRecord(`
      SELECT b.*,
        (SELECT MAX(date_finished) FROM reads WHERE reads.book_id = b.id) AS effective_date_finished
      FROM books b
      WHERE EXISTS (SELECT 1 FROM reads WHERE reads.book_id = b.id AND date_finished IS NOT NULL)
