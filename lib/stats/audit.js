...
import { countDuplicateClusters } from '../books/duplicates.js';
...
const AUDITS = [
  ...
  },
  {
    ...
    rows: [
      // ── General physical attributes ────────────────────────────────
      {
        ...
        // is_stub guard matches the Library missing=binding filter so
        // the audit count and the filtered cohort agree — clicking
        // through doesn't surface fewer rows than the dashboard claims
        // are missing. Mirrors the same guard on narrator / translator /
        // isbn / publisher / duration / page_count rows below.
        gapSql:        "COALESCE(is_stub,0)=0 AND format = 'physical' AND (binding IS NULL OR binding = '') AND COALESCE(archived,0) = 0",
        populationSql: "COALESCE(is_stub,0)=0 AND format = 'physical' AND COALESCE(archived,0) = 0",
    ...
    rows: [
      {
        ...
        gapSql:        "COALESCE(is_stub,0)=0 AND format = 'audiobook' AND id NOT IN (SELECT book_id FROM book_narrators) AND COALESCE(archived,0) = 0",
        populationSql: "COALESCE(is_stub,0)=0 AND format = 'audiobook' AND COALESCE(archived,0) = 0",
        query:         'tab=all&formats=audiobook&missing=narrator',
        wizardKey:     'narrators',
      },
      {
        label: 'Audiobooks have duration',
        gapSql:        "COALESCE(is_stub,0)=0 AND format = 'audiobook' AND duration_minutes IS NULL AND COALESCE(archived,0) = 0",
        populationSql: "COALESCE(is_stub,0)=0 AND format = 'audiobook' AND COALESCE(archived,0) = 0",
      ...
      // include audiobooks instead of skipping them.
      {
        ...
        gapSql:        "COALESCE(is_stub,0)=0 AND format = 'audiobook' AND COALESCE(is_custom,0) = 0 AND page_count IS NULL AND COALESCE(archived,0) = 0",
        populationSql: "COALESCE(is_stub,0)=0 AND format = 'audiobook' AND COALESCE(is_custom,0) = 0 AND COALESCE(archived,0) = 0",
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        gapSql:        "COALESCE(is_stub,0)=0 AND original_language IS NOT NULL AND original_language != '' AND id NOT IN (SELECT book_id FROM book_translators) AND COALESCE(archived,0) = 0",
        populationSql: "COALESCE(is_stub,0)=0 AND original_language IS NOT NULL AND original_language != '' AND COALESCE(archived,0) = 0",
      ...
      },
      {
        ...
        gapSql:        "COALESCE(is_stub,0)=0 AND COALESCE(is_custom,0) = 0 AND (format IS NULL OR format NOT IN ('ebook')) AND isbn_10 IS NULL AND isbn_13 IS NULL AND asin IS NULL AND NOT (COALESCE(year_published,0) < 1970 AND COALESCE(year_edition,0) < 1970) AND COALESCE(archived,0) = 0",
        populationSql: "COALESCE(is_stub,0)=0 AND COALESCE(is_custom,0) = 0 AND (format IS NULL OR format NOT IN ('ebook')) AND NOT (COALESCE(year_published,0) < 1970 AND COALESCE(year_edition,0) < 1970) AND COALESCE(archived,0) = 0",
        query:         'tab=all&missing=isbn',
        wizardKey:     'isbn',
      },
      {
        label: 'Books have publisher',
        gapSql:        "COALESCE(is_stub,0)=0 AND COALESCE(is_custom,0) = 0 AND (publisher IS NULL OR publisher = '') AND COALESCE(archived,0) = 0",
        populationSql: "COALESCE(is_stub,0)=0 AND COALESCE(is_custom,0) = 0 AND COALESCE(archived,0) = 0",
    ...
    rows: [
      {
        ...
        gapSql:        "status = 'finished' AND COALESCE(archived,0) = 0 AND NOT EXISTS (SELECT 1 FROM reads WHERE reads.book_id = books.id AND date_finished IS NOT NULL)",
      ...
      },
      {
        ...
        gapSql:        "status = 'reading' AND COALESCE(archived,0) = 0 AND NOT EXISTS (SELECT 1 FROM reads WHERE reads.book_id = books.id AND date_started IS NOT NULL)",
        populationSql: "status = 'reading' AND COALESCE(archived,0) = 0",
  ...
  },
  {
    ...
    rows: [
      ...
      // Duplicate / edition clusters: books sharing a normalised
      // (article-stripped) title + author set that aren't all linked
      // into one work group. Each is either alternate editions awaiting
      // a work-link or a true ingestion duplicate awaiting a merge.
      // Counts come from a computed fn (cluster detection needs JS
      // bucketing, not a WHERE clause), so this row is skipped by the
      // SUM(CASE) packing below. Population = all same-title+author
      // clusters; count = the unresolved subset. No Library `missing=`
      // filter exists for clusters, so the click-through goes straight
      // to the wizard (empty query + row-level path).
      {
        label: 'Same-work duplicates are linked or merged',
        computed: () => {
          const { total, unresolved } = countDuplicateClusters();
          return { count: unresolved, population: total };
        },
        path:      '/audit/wizard/duplicates',
        query:     '',
        wizardKey: 'duplicates',
      },
    ],
  },
  ...
  // query so AuthorsIndex floats the gap rows to the top.
  {
    ...
    rows: [
      ...
      // than living past 110. CAST(birth_date AS INTEGER) reads the
      // leading signed year — correct for "1850", "1850-06-27", and BCE
      // "-428" alike (SQLite CAST stops at the first non-numeric char).
      ...
      // implausible records, not still-living centenarians.
      {
        ...
        gapSql:        "birth_date IS NOT NULL AND death_date IS NULL AND CAST(birth_date AS INTEGER) < CAST(strftime('%Y','now') AS INTEGER) - 110 AND EXISTS (SELECT 1 FROM book_authors ba WHERE ba.author_id = authors.id)",
...
export function getAudit() {
  // Per-table SUM(CASE) packing. The old shape issued 2 COUNT queries
  // per row (gap + population), which on a 38-row audit meant 76 prepared
  // statements per call. Most rows share a source table, so
  // we group by `group.from` (defaulting to books), build one packed
  // query per table that produces every gap/population count via
  // SUM(CASE WHEN ... THEN 1 ELSE 0 END), and resolve each row off the
  // single result. One full-table scan is faster than 38 index-driven
  // counts here — the per-statement overhead dominates the actual count
  // work on a 1900-row library.
  const tableGroups = new Map();
  for (const group of AUDITS) {
    const table = group.from || 'books';
    if (!tableGroups.has(table)) tableGroups.set(table, []);
    tableGroups.get(table).push(group);
  }
  // Flat list of every row, parallel to the SUM columns. Each gets a
  // numeric slot used to alias its g{i}/p{i} columns and read them back.
  const slot = new Map(); // row reference → index
  let next = 0;
  const counts = new Map(); // row reference → { count, population }
  for (const [table, groups] of tableGroups) {
    const sums = [];
    for (const group of groups) {
      for (const row of group.rows) {
        // Computed rows derive their counts from a function instead of
        // gap/population WHERE clauses — resolve directly, skip packing.
        if (row.computed) {
          counts.set(row, row.computed());
          continue;
        }
        const i = next++;
        slot.set(row, i);
        sums.push(`SUM(CASE WHEN ${row.gapSql} THEN 1 ELSE 0 END) AS g${i}`);
        sums.push(`SUM(CASE WHEN ${row.populationSql} THEN 1 ELSE 0 END) AS p${i}`);
      }
    }
    if (!sums.length) continue;
    const result = db.prepare(`SELECT ${sums.join(', ')} FROM ${table}`).get();
    for (const group of groups) {
      for (const row of group.rows) {
        if (row.computed) continue;
        const i = slot.get(row);
        counts.set(row, { count: result[`g${i}`] ?? 0, population: result[`p${i}`] ?? 0 });
      }
    }
  }
  ...
  const audit = AUDITS.map(group => ({
    ...
    rows: group.rows.map(row => {
      const { count, population } = counts.get(row);
      totalGaps       += count;
