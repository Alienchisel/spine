...
export function getAudit() {
  ...
  const audit = AUDITS.map(group => ({
    ...
    rows: group.rows.map(row => {
      const { c: count } = db.prepare(`SELECT COUNT(*) AS c FROM books WHERE ${row.gapSql}`).get();
      const { c: pop }   = db.prepare(`SELECT COUNT(*) AS c FROM books WHERE ${row.populationSql}`).get();
      totalGaps       += count;
      totalPopulation += pop;
      return { label: row.label, count, population: pop, query: row.query };
    }),
  }));
