...
// Rated-and-paged books for /data-viz's dot-dash scatter. Returns one
// row per book with both a rating and a positive page_count.
export function getPageRatingScatter() {
  return db.prepare(`
    SELECT id, title, page_count, rating, format
    FROM books
...
// Per-book reading lag — days between acquisition_date and
// date_finished, for /data-viz's lag-distribution histogram. Books
// without both dates are excluded; partial acquisition_dates (year or
// year-month only) are normalised to mid-period for julianday
// arithmetic so an "acquired in 2019" book finished mid-2019 doesn't
// register an extreme lag from a 1-Jan default.
export function getReadingLag() {
  return db.prepare(`
    SELECT
      id,
      title,
      ...
      date_finished,
      ROUND(
        julianday(date_finished) - julianday(
          CASE
            WHEN length(acquisition_date) >= 10 THEN acquisition_date
            WHEN length(acquisition_date) = 7  THEN acquisition_date || '-15'
            WHEN length(acquisition_date) = 4  THEN acquisition_date || '-07-01'
            ELSE NULL
          END
        )
      ) AS lag_days
    FROM books
    WHERE date_finished     IS NOT NULL AND date_finished     != ''
      AND acquisition_date  IS NOT NULL AND acquisition_date  != ''
      AND COALESCE(archived, 0) = 0
    ORDER BY lag_days
  `).all().filter(r => r.lag_days != null);
}
