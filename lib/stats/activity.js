...
export function getActivityStats() {
  ...
  // "books finished this year" hero stat. 'now','localtime' matches the
  // rest of the dashboard (todayPages, reading_log dates are localtime),
  // so the year boundary doesn't drift by up to a day around New Year's.
  const thisYearBooks = db.prepare(`
    SELECT COUNT(DISTINCT book_id) AS total FROM reads
    WHERE date_finished IS NOT NULL
      AND strftime('%Y', date_finished) = strftime('%Y', 'now', 'localtime')
  `).get().total;
