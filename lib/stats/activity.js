...
export function getLibraryTrajectory() {
  ...
  const acq = db.prepare(`
    ...
    GROUP BY month
  `).all();
  // Library trajectory counts each book once at its *first* finish — the
  // moment it entered the "finished" pile. A re-read in 2025 of a book
  // first finished in 2020 must not add a second tick to the 2025 month
  // (that would distort the cumulative unique-books-finished trajectory).
  // first_finished = MIN(reads.date_finished) ?? books.date_finished —
  // the COALESCE fallback bridges books with a date on the row but no
  // reads row (pre-Phase-1 imports); Phase 3 drops it.
  const fin = db.prepare(`
    SELECT month, COUNT(*) AS n FROM (
      SELECT ${monthExpr.replace(/\bd\b/g, 'first_finished')} AS month FROM (
        SELECT COALESCE(
          (SELECT MIN(date_finished) FROM reads WHERE reads.book_id = b.id),
          b.date_finished
        ) AS first_finished
        FROM books b
      )
      WHERE first_finished IS NOT NULL AND first_finished != ''
    )
    WHERE month IS NOT NULL
    GROUP BY month
  `).all();
