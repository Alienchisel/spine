...
// formatPartialDate (shared with the client via shared/dates.js) renders
// recently_finished / year_in_review tile sublabels as "1893" / "July
// 1893" / "July 18, 1893" instead of raw ISO strings.
...
function recentlyFinished(limit) {
  // effective_date_finished = MAX(reads.date_finished). Phase 3 dropped
  // the book-column fallback the bridge needed.
  const rows = db.prepare(`
    SELECT b.id, b.title, b.cover_path,
     ...
     WHERE b.status = 'finished'
       AND EXISTS (SELECT 1 FROM reads WHERE reads.book_id = b.id AND date_finished IS NOT NULL)
...
function yearInReview(year) {
  ...
  // year. The sublabel surfaces the matching read's date (MIN within
  // the year — earliest finish in this year for the book). Books
  // finished multiple times in the same year still show once —
  // duplicate covers in a year-in-review collage would feel weird; the
  // per-read history is on BookDetail.
  const rows = db.prepare(`
    SELECT b.id, b.title, b.cover_path,
      (SELECT MIN(date_finished) FROM reads WHERE reads.book_id = b.id AND date_finished LIKE ?) AS effective_date_finished
      FROM books b
     WHERE b.status = 'finished'
       AND EXISTS (SELECT 1 FROM reads WHERE reads.book_id = b.id AND date_finished LIKE ?)
