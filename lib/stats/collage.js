...
export const ALLOWED_MODES   = Object.freeze([
  ...
  'hand_curated', 'top_loved', 'top_rated',
]);
...
// "User-curated list of specific book IDs, rendered in the order given."
// Order matters and SQLite's IN() doesn't preserve it, so we fetch the
// set and reorder in JS. Unknown IDs are silently dropped — a stale
// bookmarked URL with a since-deleted book just shows the survivors
// instead of erroring.
function handCurated(bookIds, limit) {
  if (!bookIds || bookIds.length === 0) return [];
  const limited = bookIds.slice(0, limit);
...
export function computeCollage({ mode, period, size, series, year, books }) {
  ...
  switch (mode) {
    ...
    case 'hand_curated':      tiles = handCurated(books, limit);      break;
