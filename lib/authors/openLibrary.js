...
// authors.photo_path. Filename embeds the authorId + a timestamp so a
// later refresh writes a fresh file (browsers won't show a stale
// cached portrait).
export async function downloadAuthorPhoto(authorId, photoId) {
  await fs.mkdir(UPLOADS_DIR, { recursive: true });
  const buf = await getBuffer(`${PHOTO_BASE}/${photoId}-L.jpg`);
  // OL returns a 1x1 placeholder for missing photos. Filter those out
  // so we don't store an empty portrait that the UI would treat as a
  // real one.
  if (buf.length < 1024) return null;
  const filename = `${authorId}-${Date.now()}.jpg`;
  await fs.writeFile(path.join(UPLOADS_DIR, filename), buf);
  return `/uploads/authors/${filename}`;
}

// Best-effort delete of a previous photo file when a refresh replaces
// it. Errors are swallowed — leaving the old file is harmless and the
// next backup picks both up.
export async function deleteAuthorPhoto(photoPath) {
  ...
  if (!/^[\w.-]+$/.test(filename)) return; // path-traversal guard
  try {
