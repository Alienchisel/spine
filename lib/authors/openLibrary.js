...
import { assertPublicHttpsUrl, fetchFollowingRedirects } from '../security/url.js';
...
const USER_AGENT  = 'Spine/1.0 (personal library tracker; +https://github.com/Alienchisel/spine)';

// One JSON fetch attempt. Classifies failures the same way fetchBufferOnce
// does: AbortError + TypeError = transient network failure; 5xx + 429 =
// transient HTTP; 4xx = permanent.
async function fetchJSONOnce(url, ms) {
  ...
  if (!res.ok) {
    const e = new Error(`OL ${res.status}: ${url}`);
...
// Abortable JSON fetch with one retry on transient failures. Default
// 8s timeout is fine for one-key author-detail lookups; the multi-token
// search endpoint (searchAuthor / searchAuthorsMulti) passes ms=15000 —
// OL's /search index routinely spikes past 5s under load. The retry
// absorbs the same shape of flake fetchBufferOnce already does on the
// photo path: a 502 / abort on the first attempt that resolves cleanly
// 200ms later. Without it, the portrait wizard surfaces a 'Search
// failed' banner on otherwise-fine queries (Jane Jacobs in particular).
async function getJSON(url, ms = 8000) {
  try {
    return await fetchJSONOnce(url, ms);
  } catch (err) {
    if (!err.transient) throw err;
    await new Promise(r => setTimeout(r, 300));
    return fetchJSONOnce(url, ms);
  }
}

// One buffer fetch attempt. Classifies failures into transient (worth a
// retry) vs permanent: AbortError + TypeError = transient at the network
// layer; 5xx/429 = transient at the HTTP layer; 4xx = permanent. Mirrors
// the policy in lib/books/covers.js fetchCoverOnce.
async function fetchBufferOnce(url) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 8000);
  let res;
  try {
    // Manual-redirect fetch re-validating each hop through the SSRF guard;
    // a plain fetch() would follow a redirect to a private address that the
    // guard never got to inspect (see fetchFollowingRedirects).
...
// Retry-once wrapper. covers.openlibrary.org redirects to archive.org for
// the actual image, and archive.org is sporadically slow / refuses
// connections under load — a single retry after a short delay absorbs
// nearly all of those flakes silently. Persistent failure tags the error
// with networkFailure so route handlers can surface a backend-specific
// message instead of a generic "Failed to fetch photo".
async function getBuffer(url) {
  try {
    ...
    try {
      ...
      e.networkFailure = retryErr.networkFailure;
      throw e;
    }
  }
}
...
export async function downloadAuthorPhotoByUrl(authorId, url) {
  // SSRF guard — same shape as the cover-url path. Rejects non-https
  // and any hostname that resolves to loopback / link-local / RFC1918
  // private ranges; the saved photo would otherwise be retrievable via
  // GET /uploads/<filename>, turning the proxy into an exfil channel.
  try { await assertPublicHttpsUrl(url); }
  catch (err) { const e = new Error(err.message || 'Invalid URL'); e.networkFailure = false; e.status = err.status || 400; throw e; }
