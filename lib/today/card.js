...
export function pickTodayCard(dateStr, { peek = false } = {}) {
  ...
  for (const type of CARD_TYPES) {
    ...
    if (QUEUE_CARD_TYPES.includes(type)) {
      ...
      // Three sources match the 'finished' event during the Phase 2
      // transition: any reads-row finish (covers re-reads — the whole
      // point of this refactor) plus the book-column fallback for rows
      // without a reads row (pre-cascade imports). Phase 3 drops the
      // book-column source.
      const rows = db.prepare(`
...
export function computeCardMeta(type, bookId, dateStr) {
  ...
  if (type === 'personal_anniversary') {
    ...
    if (!row) return {};
    // Phase 2: candidates for 'finished' come from reads (covers re-reads)
    // first, then the book column as fallback. The first match wins —
    // matching the cohort query's UNION-ALL ordering so the displayed
    // event_year corresponds to the row the cohort actually selected.
    ...
    if (row.date_finished) finishedCandidates.push(row.date_finished);
    const dateYear = parseInt(String(dateStr).slice(0, 4), 10) || 0;
