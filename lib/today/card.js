...
// The three date-windowed cohorts anchor their window on the VIEWED date
// (bound `?` = dateStr), not date('now'). For the live card dateStr IS
// today, so behaviour is unchanged; for a ?date= preview the window is
// relative to the day being previewed (more correct); and for tests it's
// fully deterministic — date('now') leaked the real wall clock past
// TODAY_NOW_OVERRIDE, so cohort eligibility (and thus the seeded card
// winner) drifted day to day, which time-bombed the today suite. dateStr
// is a validated full 'YYYY-MM-DD' local date, so no 'localtime' modifier
// is needed (applying it to a bare date would shift it by a day). See
// DATE_SCOPED_COHORTS below — those get .all(dateStr).
const COHORT_SQL = {
  loved_resurface: `
    ...
    WHERE b.loved = 1
      ...
      AND (
        SELECT MAX(date_finished) FROM reads
         WHERE book_id = b.id
           AND date_finished IS NOT NULL
           AND length(date_finished) = 10
      ) < date(?, '-180 days')
    ORDER BY b.id
  `,
  slow_burn: `
    ...
    WHERE b.status = 'reading'
      ...
      AND EXISTS (
        ...
        WHERE r.book_id = b.id
          ...
          AND length(r.date_started) = 10
          AND r.date_started < date(?, '-30 days')
      )
    ORDER BY b.id
  `,
  recent_acquisition: `
    ...
    WHERE owned = 1
      ...
      AND acquisition_date >= date(?, '-14 days')
      AND status = 'unread'
...
// Cohorts whose SQL takes a single bound date (the viewed dateStr) as the
// anchor for their `date(?, '-N days')` window. Every other COHORT_SQL
// entry is parameterless. Kept as a set so the compute loop knows which
// cohorts to pass dateStr to.
const DATE_SCOPED_COHORTS = Object.freeze(new Set(['loved_resurface', 'slow_burn', 'recent_acquisition']));
...
export function pickTodayCard(dateStr, { peek = false } = {}) {
  ...
  for (const type of CARD_TYPES) {
    ...
    if (QUEUE_CARD_TYPES.includes(type)) {
      ...
      // 'finished' event covers any read of any book on this MM-DD
      // (re-reads count, the whole point of the refactor). 'acquired'
      // event matches book.acquisition_date the same way. ORDER BY
      // event_date ASC makes the cohort iteration deterministic — the
      // byId map below dedupes per book and the render-side resolution
      // walks reads in the same order, so both queries agree on which
      // event_date wins when a book has multiple MM-DD matches.
      const rows = db.prepare(`
        SELECT b.id, 'finished' AS event, r.date_finished AS event_date
          FROM books b
          JOIN reads r ON r.book_id = b.id
         WHERE r.date_finished IS NOT NULL
           AND substr(r.date_finished, 6, 5) = substr(?, 6, 5)
           AND COALESCE(b.archived, 0) = 0
        UNION ALL
      ...
      const stmt = db.prepare(COHORT_SQL[type]);
      ids = (DATE_SCOPED_COHORTS.has(type) ? stmt.all(dateStr) : stmt.all())
        .map(r => r.id)
  ...
  // Future-dated requests are side-effect-free previews: return the
  // computed pick but persist nothing. The client never asks ahead of
  // today, but exploratory ?date= requests (dev poking at the live DB)
  // used to lock each future day to whatever they computed — 88 such
  // pre-locked rows were hand-deleted on 2026-07-04. ISO strings
  // compare correctly as strings. TODAY_NOW_OVERRIDE is the test
  // clock: the suite's date-sweep fixtures (2026-11, 2027-03, …) rely
  // on persistence at arbitrary dates against the in-memory DB, so
  // createTestServer pins "today" far in the future.
  const isFuture = dateStr >
    (process.env.TODAY_NOW_OVERRIDE || new Date().toLocaleDateString('en-CA'));
...
export function computeCardMeta(type, bookId, dateStr) {
  ...
  if (type === 'series_next_volume') {
    ...
    // date_finished sourced from reads (most-recent finish per series
    // sibling) — books.date_finished was dropped in Phase 3.
    const prev = db.prepare(`
      SELECT b.title, b.series_number AS volume,
             (SELECT MAX(date_finished) FROM reads WHERE reads.book_id = b.id) AS date_finished,
             CAST(julianday(?) - julianday((SELECT MAX(date_finished) FROM reads WHERE reads.book_id = b.id)) AS INTEGER) AS days_since_prev
        FROM books b
       WHERE b.series = ?
         AND b.status = 'finished'
         AND b.series_number IS NOT NULL
         AND COALESCE(b.archived, 0) = 0
       ORDER BY b.series_number DESC, b.id DESC
       LIMIT 1
  ...
  if (type === 'personal_anniversary') {
    ...
    const row = db.prepare(
      'SELECT acquisition_date FROM books WHERE id = ?'
    ).get(bookId);
    if (!row) return {};
    // Phase 3: every 'finished' candidate lives in the reads table.
    // ORDER BY ASC + break-on-first picks the *earliest* MM-DD match.
    // This aligns with the cohort selector above: that loop uses an
    // ASC iteration order and a `continue`-on-existing-finished gate,
    // which means the first finished event encountered per book is
    // the one stored in byId. Without this alignment, two
    // independently-ordered queries could disagree on which of
    // multiple MM-DD anniversaries a book is being surfaced for, and
    // the card would render an event_year that doesn't correspond to
    // the read the cohort actually picked.
    const finishedCandidates = db.prepare(
      'SELECT date_finished FROM reads WHERE book_id = ? AND date_finished IS NOT NULL ORDER BY date_finished ASC, id ASC'
    ).all(bookId).map(r => r.date_finished);
    const dateYear = parseInt(String(dateStr).slice(0, 4), 10) || 0;
    ...
    let match = null;
    for (const fd of finishedCandidates) {
      match = tryEvent(fd, 'finished');
      if (match) break;
    }
    if (!match) match = tryEvent(row.acquisition_date, 'acquired');
    return match || {};
  }
  if (type === 'anniversary') {
  ...
  if (type === 'author_anniversary') {
    ...
    const row = db.prepare(`
      SELECT a.name, a.gender, a.birth_date, a.death_date,
        (SELECT COUNT(DISTINCT ba2.book_id)
