    ...
    WHERE b.status = 'reading'
      ...
      AND EXISTS (
        ...
        WHERE r.book_id = b.id
          ...
          AND r.date_started < date('now', 'localtime', '-30 days')
      )
    ORDER BY b.id
  `,
  recent_acquisition: `
...
// IDs the user has actively snoozed via a card's "Snooze" action.
// snoozed_until is a calendar date stored on the row; while the viewed
// date is at or before that date, the book stays excluded from cohort
// picks. The repetition guard handles same-book back-to-back within
// 14 days; snoozes are the longer-tail "I've seen enough of this" lever.
function snoozedBookIds(dateStr) {
  const rows = db.prepare(
    'SELECT book_id FROM today_snoozes WHERE snoozed_until >= ?'
  ).all(dateStr);
  return new Set(rows.map(r => r.book_id));
}
...
export function pickTodayCard(dateStr, { peek = false } = {}) {
  ...
  // Merge the repetition guard with active snoozes into one exclusion
  // set so every cohort filter runs against the union. Cohort SQL
  // filters by id ∉ recent in JS post-fetch (it can't see the JS Set),
  // so accumulating both signals here keeps the contract uniform.
  const recent  = recentlySurfaced(dateStr);
  for (const id of snoozedBookIds(dateStr)) recent.add(id);
  const cohorts = {};
  const eligibleTypes = [];
