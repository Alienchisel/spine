    ...
    WHERE b.loved = 1
      ...
      AND EXISTS (
        ...
        WHERE r.book_id = b.id
          AND r.date_finished IS NOT NULL
          AND length(r.date_finished) = 10
          AND r.date_finished < date('now', 'localtime', '-180 days')
      )
    ORDER BY b.id
  `,
  slow_burn: `
...
export function pickTodayCard(dateStr, { peek = false } = {}) {
  ...
  for (const type of CARD_TYPES) {
    ...
    if (QUEUE_CARD_TYPES.includes(type)) {
      ...
      // event matches book.acquisition_date the same way.
      const rows = db.prepare(`
...
export function computeCardMeta(type, bookId, dateStr) {
  ...
  if (type === 'personal_anniversary') {
    ...
    const finishedCandidates = db.prepare(
      'SELECT date_finished FROM reads WHERE book_id = ? AND date_finished IS NOT NULL'
