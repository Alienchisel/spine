...
// 'connection' is the AI-shaped Cross-author thematic card type. Its
// cohort isn't a book-centric cohort (no first-author bucket, no
// status filter) — it's the queue of pre-generated candidates from
// today_card_queue that haven't been served yet. Selection happens
// through the same seeded-mod dance as the book types, but persistence
// goes onto the queue row itself (served_at + served_date) rather
// than today_card_history. See migration 073 and the connection
// branches below for the details.
export const CARD_TYPES = Object.freeze([...Object.keys(COHORT_SQL), 'connection']);
...
//   { type: 'connection', queueId }   for AI-shaped Connection cards
//   null                         when nothing is eligible
// The result is persisted on first compute — book-centric to
// today_card_history, connection to today_card_queue.served_at /
...
export function pickTodayCard(dateStr) {
  ...
  // Or a Connection? Connection state lives on the queue row itself
  // rather than today_card_history (no book_id to key off).
  const existingConn = db.prepare(
    'SELECT id FROM today_card_queue WHERE served_date = ?'
  ).get(dateStr);
  if (existingConn) {
    return { type: 'connection', queueId: existingConn.id };
  }
  ...
  for (const type of CARD_TYPES) {
    ...
    if (type === 'connection') {
      ids = db.prepare(
        'SELECT id FROM today_card_queue WHERE served_at IS NULL ORDER BY id'
      ).all().map(r => r.id);
    } else {
