...
// Diff two row-count snapshots and classify concerning transitions.
//   wiped  — table existed before AND after, was non-empty, now empty
//   shrunk — table existed before AND after, was non-empty, lost ≥50%
// Tables present in `before` but absent from `after` are intentionally
// skipped: a migration that drops or renames a table is not the bug
// we're guarding against. The cascade case the check exists for keeps
// the junction tables in place — only their rows vanish.
export function diffRowCounts(before, after) {
  ...
  for (const [table, beforeCount] of before) {
    ...
    if (afterCount === 0) {
      ...
      shrunk.push(`${table}: ${beforeCount} → ${afterCount}`);
    }
  }
  return { wiped, shrunk };
}
