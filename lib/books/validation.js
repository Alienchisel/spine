...
export function validateBook(body) {
  ...
  // showcase_position is an integer 1–5 or null/absent. Reject 0, 6+, floats,
  // and non-numeric values loudly so a stray API caller can't quietly stuff
  // an out-of-range value that the UI later renders into a non-existent slot.
  if (body.showcase_position != null && body.showcase_position !== '') {
    const n = Number(body.showcase_position);
    if (!Number.isInteger(n) || n < 1 || n > 5) errors.push('showcase_position must be an integer 1–5');
  }
