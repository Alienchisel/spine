...
async function fetchCoverOnce(url) {
  ...
  let response;
  try {
    // Manual-redirect fetch that re-validates every hop against the SSRF
    // guard — a plain fetch() follows redirects to private addresses, and
    // the guard only ever saw the first URL (see fetchFollowingRedirects).
