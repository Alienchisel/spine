...
export function buildFilterConditions(query) {
  ...
  if (query.field && query.value != null) {
    ...
    if (f === 'tag') {
      ...
      // Match any read of this book that finished in the given year.
      // Phase 3 dropped books.date_finished so this no longer needs
      // the dual-source check Phase 2 carried.
      conditions.push("EXISTS (SELECT 1 FROM reads WHERE reads.book_id = books.id AND date_finished LIKE ?)");
      params.push(v + '%');
  ...
  if (fmts.length) {
    ...
    // Short-circuit when every possible value (all enum + empty) is
    // selected — no filter matches every row, so a WHERE clause would
    // be wasted SQL. Parallels the fictions[] / sourceTypes[] handling
    // below and closes the shape-inconsistency the 2026-07-15 sweep
    // flagged.
    const allSelected = hasEmpty && real.length === ENUM_VALUES.format.length;
    if (allSelected) { /* no-op */ }
    else if (hasEmpty && real.length) { conditions.push(`(format IS NULL OR format IN (${real.map(() => '?').join(',')}))`); params.push(...real); }
    else if (hasEmpty) conditions.push("format IS NULL");
    else { conditions.push(`format IN (${real.map(() => '?').join(',')})`); params.push(...real); }
  }

  const fics = [].concat(query.fictions || []).filter(Boolean);
  if (fics.length) {
    // Fiction status is a small tristate (fiction=1, fiction=0, fiction IS
    // NULL). Same shape as formats: 'empty' sentinel means "unset", real
    // values are 'fiction' / 'nonfiction'. Selecting all three collapses
    // to no filter (matches every row) — the UI hides the section then.
    const wantFic  = fics.includes('fiction');
    const wantNF   = fics.includes('nonfiction');
    const wantNull = fics.includes('empty');
    const clauses = [];
    if (wantFic)  clauses.push("fiction = 1");
    if (wantNF)   clauses.push("fiction = 0");
    if (wantNull) clauses.push("fiction IS NULL");
    if (clauses.length && clauses.length < 3) conditions.push(`(${clauses.join(' OR ')})`);
  }

  const stypes = [].concat(query.sourceTypes || []).filter(Boolean);
  if (stypes.length) {
    // Same tristate shape as fictions. Only meaningful for non-fiction
    // (source_type is nulled on fiction rows by validation), but the
    // filter itself doesn't gate — if a user selects primary while also
    // filtering to fiction, they'll get zero results, which is honest.
    const wantP  = stypes.includes('primary');
    const wantS  = stypes.includes('secondary');
    const wantNu = stypes.includes('empty');
    const cls = [];
    if (wantP)  cls.push("source_type = 'primary'");
    if (wantS)  cls.push("source_type = 'secondary'");
    if (wantNu) cls.push("source_type IS NULL");
    if (cls.length && cls.length < 3) conditions.push(`(${cls.join(' OR ')})`);
  }

  const bndgs = [].concat(query.bindings || []).filter(Boolean);
  if (bndgs.length) {
    const hasEmpty = bndgs.includes('empty');
    const real = bndgs.filter(b => b !== 'empty');
    // Same short-circuit as formats[] — matches every row when all
    // possible values are selected.
    const allSelected = hasEmpty && real.length === ENUM_VALUES.binding.length;
    if (allSelected) { /* no-op */ }
    else if (hasEmpty && real.length) { conditions.push(`(binding IS NULL OR binding IN (${real.map(() => '?').join(',')}))`); params.push(...real); }
    else if (hasEmpty) conditions.push("binding IS NULL");
    else { conditions.push(`binding IN (${real.map(() => '?').join(',')})`); params.push(...real); }
  }

  if (query.stub === 'true')  conditions.push("is_stub = 1");
  if (query.stub === 'false') conditions.push("(is_stub IS NULL OR is_stub = 0)");
  ...
  // Two language filters, paired in the UI as Original (work-language)
  // and Edition (the language the user actually holds the book in).
  //   - originalLanguages → `original_language` partitions the
  //     translated corpus (Greek Loebs, German philosophy, French
  //     literature, etc.). 994 books null this — the bulk are
  //     untouched English originals.
  //   - editionLanguages → `language` is the reader's view. 98%
  //     English; the remaining ~85 books are the user's French / Greek
  //     facing-page Loeb / Italian / Latin / Japanese / German editions.
  // Both filters share the multi-select + 'empty' sentinel pattern of
  // series/publishers/sources.
  const olangs = [].concat(query.originalLanguages || []).filter(Boolean);
  if (olangs.length) {
    const hasEmpty = olangs.includes('empty');
    const real = olangs.filter(l => l !== 'empty');
    if (hasEmpty && real.length) { conditions.push(`(original_language IS NULL OR original_language IN (${real.map(() => '?').join(',')}))`); params.push(...real); }
    else if (hasEmpty) conditions.push("(original_language IS NULL OR original_language = '')");
    else { conditions.push(`original_language IN (${real.map(() => '?').join(',')})`); params.push(...real); }
  }

  const elangs = [].concat(query.editionLanguages || []).filter(Boolean);
  if (elangs.length) {
    const hasEmpty = elangs.includes('empty');
    const real = elangs.filter(l => l !== 'empty');
    if (hasEmpty && real.length) { conditions.push(`(language IS NULL OR language IN (${real.map(() => '?').join(',')}))`); params.push(...real); }
    else if (hasEmpty) conditions.push("(language IS NULL OR language = '')");
    else { conditions.push(`language IN (${real.map(() => '?').join(',')})`); params.push(...real); }
  }
  ...
  for (const m of missing) {
    ...
    // Finished reads with no recorded finish date on any reads row —
    // usually historical imports or hand-edited status changes. Phase 3
    // dropped books.date_finished so the dual-source check from Phase 2
    // collapses to a single NOT EXISTS.
    else if (m === 'date_finished') conditions.push(
      "status = 'finished' AND NOT EXISTS (SELECT 1 FROM reads WHERE reads.book_id = books.id AND date_finished IS NOT NULL)"
    );
    // Currently-reading books with no recorded start date on any
    // reads row — entry-side parallel.
    else if (m === 'date_started')  conditions.push(
      "status = 'reading' AND NOT EXISTS (SELECT 1 FROM reads WHERE reads.book_id = books.id AND date_started IS NOT NULL)"
    );
...
// Article-stripping, diacritic-folded title sort: "The Odyssey" sorts
// under O (article stripped), "Étranger" under E (diacritic folded via
// nrm() — which also lowercases, NFD-decomposes, and folds non-
// decomposing ligatures like ł/đ/ø/æ/œ/ß). Used by the library list
// AND every shelf-route ORDER BY that falls back to title, so the two
// browsing surfaces feel consistent. Pass a column reference (`b.title`,
// `COALESCE(b.series, b.title)`, etc.) to scope to a join alias or a
// derived expression. The inner LIKEs stay as LOWER(): articles are
// ASCII and LOWER is cheaper than nrm; the outer wrap is what folds.
export function titleSortExpr(col = 'title') {
  return `nrm(CASE WHEN LOWER(${col}) LIKE 'the %' THEN SUBSTR(${col},5) WHEN LOWER(${col}) LIKE 'an %' THEN SUBSTR(${col},4) WHEN LOWER(${col}) LIKE 'a %' THEN SUBSTR(${col},3) ELSE ${col} END)`;
}
...
export function buildOrderBy(sort, field, seed) {
  ...
  // /browse/year_finished/X sorts chronologically within the year-bucket
  // it was filtered down to. Uses the most-recent finish per book.
  if (field === 'year_finished') return `COALESCE((SELECT MAX(date_finished) FROM reads WHERE reads.book_id = books.id), '') ASC, ${titleSort} ASC`;
  if (field === 'year_acquired') return `acquisition_date ASC, ${titleSort} ASC`;
  ...
  switch (sort) {
    ...
    // 'started' / 'finished' sort the most recent reads-row date to
    // the top. The subquery is index-supported by
    // idx_reads_book_id_date_finished (migration 077).
    case 'started':  return `COALESCE((SELECT MAX(date_started)  FROM reads WHERE reads.book_id = books.id), '') DESC, ${titleSort} ASC`;
    case 'finished': return `COALESCE((SELECT MAX(date_finished) FROM reads WHERE reads.book_id = books.id), '') DESC, ${titleSort} ASC`;
    case 'acquired': return `COALESCE(acquisition_date,'') DESC, ${titleSort} ASC`;
    ...
    case 'last_logged': return `COALESCE((SELECT MAX(date) FROM reading_log WHERE book_id = books.id), '') DESC, updated_at DESC, ${titleSort} ASC`;
    case 'random': {
    ...
    default:         return `updated_at DESC, ${titleSort} ASC`;
  }
}
