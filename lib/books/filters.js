...
export function buildFilterConditions(query) {
  ...
  if (query.field && query.value != null) {
    ...
    if (f === 'tag') {
      ...
      // OR the book-column date (Phase 2 transition fallback for books
      // imported with a date but no reads row). Pre-Phase-2 this only
      // checked books.date_finished, which holds the first-read date —
      // so a 2025 re-read of a 2020 finish wouldn't surface under
      // year=2025. The EXISTS catches re-reads; the LIKE keeps
      // book-column-only rows working until Phase 3 drops them.
      conditions.push("(EXISTS (SELECT 1 FROM reads WHERE reads.book_id = books.id AND date_finished LIKE ?) OR date_finished LIKE ?)");
      params.push(v + '%', v + '%');
  ...
  for (const m of missing) {
    ...
    // Finished reads with no recorded finish date — usually historical
    // imports or hand-edited status changes. Must check both sources
    // during the Phase 2 transition: the book column AND the reads
    // table. Either one having a date excludes the book.
    else if (m === 'date_finished') conditions.push(
      "status = 'finished' AND date_finished IS NULL AND NOT EXISTS (SELECT 1 FROM reads WHERE reads.book_id = books.id AND date_finished IS NOT NULL)"
    );
    // Currently-reading books with no recorded start date — same
    // dual-source check on the entry side.
    else if (m === 'date_started')  conditions.push(
      "status = 'reading' AND date_started IS NULL AND NOT EXISTS (SELECT 1 FROM reads WHERE reads.book_id = books.id AND date_started IS NOT NULL)"
    );
...
export function buildOrderBy(sort, field, seed) {
  ...
  // it was filtered down to. Uses the most-recent finish per book with
  // book-column fallback (Phase 2 bridge — re-reads sort right while
  // book-column-only rows still work). Phase 3 drops the fallback.
  if (field === 'year_finished') return "COALESCE((SELECT MAX(date_finished) FROM reads WHERE reads.book_id = books.id), date_finished, '') ASC";
  ...
  switch (sort) {
    ...
    // 'started' / 'finished' sort the most recent reads-row date to the
    // top, falling back to the book column when no reads row carries
    // the date. The book-column fallback is the Phase 2 transition
    // bridge: re-reads sort under the new reads-table semantics, while
    // books in unusual states (date set on the book row but no reads
    // row — historically possible via raw imports) still surface in
    // the right bucket. Phase 3 drops the book columns and this
    // collapses back to the inner subquery. The subquery is index-
    // supported by idx_reads_book_id_date_finished (migration 077).
    case 'started':  return `COALESCE((SELECT MAX(date_started)  FROM reads WHERE reads.book_id = books.id), date_started,  '') DESC, ${titleSort} ASC`;
    case 'finished': return `COALESCE((SELECT MAX(date_finished) FROM reads WHERE reads.book_id = books.id), date_finished, '') DESC, ${titleSort} ASC`;
