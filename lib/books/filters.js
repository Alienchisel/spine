...
export function buildFilterConditions(query) {
  ...
  // showcase=1 narrows to books currently occupying a slot on /showcase.
  // Pairs with sort=showcase to render the row in rank order.
  if (query.showcase === '1' || query.showcase === 'true') conditions.push("showcase_position IS NOT NULL");
...
export function buildOrderBy(sort, field, seed) {
  ...
  switch (sort) {
    ...
    // /showcase reads the user's hand-ranked top picks. Books outside the
    // showcase land last (NULL → 99 via COALESCE) so a pairing of
    // showcase=1 with this sort renders 1, 2, 3, … in left-to-right order.
    case 'showcase':    return `COALESCE(showcase_position, 99) ASC, ${titleSort} ASC`;
    case 'random': {
