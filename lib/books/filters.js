...
export const VIRTUAL_TAG_RULES = Object.freeze([
  ...
  },
  {
    ...
    // qualifies the same way a 500-page ebook does.
    test: (book) => book.page_count >= 500 || book.duration_minutes >= 840,
    sql: "(page_count >= 500 OR duration_minutes >= 840)",
  },
  {
    name: 'Short',
...
export function buildFilterConditions(query) {
  ...
  else if (tab === 'owned')      conditions.push("owned = 1 AND COALESCE(is_custom,0) = 0 AND COALESCE(acquisition_source,'') != 'Internet'");
