...
export function buildOrderBy(sort, field, seed) {
  ...
  switch (sort) {
    ...
    // condition expressible without `NULLS LAST` syntax.
    case 'year_published':      return `COALESCE(year_published, 99999) ASC, ${titleSort} ASC`;
    case 'year_published_desc': return `year_published DESC NULLS LAST, ${titleSort} ASC`;
