...
export function buildFilterConditions(query) {
  ...
  // finished, currently in-progress, or with a prior read on record.
  // Used by the Stats hero strip so "Pages read" / "Hours listened"
  // click through to the actual cohort the numbers came from rather
  // than the entire format catalog.
  if (query.progress === 'any') {
    conditions.push(
      "(status = 'finished' OR COALESCE(current_page,0) > 0 OR COALESCE(current_minutes,0) > 0 OR COALESCE(read_count,0) > 0)"
    );
  }
  ...
  for (const m of missing) {
    ...
    else if (m === 'binding')    conditions.push("format = 'physical' AND (binding IS NULL OR binding = '')");
    else if (m === 'translator') conditions.push("original_language IS NOT NULL AND original_language != '' AND id NOT IN (SELECT book_id FROM book_translators)");
    ...
    // is_custom guard mirrors missing=isbn: custom (user-assembled)
    // books don't have publishers / acquisition sources / acquisition
    // dates as a matter of model — surfacing them in a "missing X"
    // maintenance filter is noise.
    else if (m === 'publisher')  conditions.push("COALESCE(is_custom,0)=0 AND (publisher IS NULL OR publisher = '')");
    ...
    else if (m === 'pages')      conditions.push("COALESCE(format,'') != 'audiobook' AND page_count IS NULL");
    else if (m === 'duration')   conditions.push("format = 'audiobook' AND duration_minutes IS NULL");
    ...
    else if (m === 'page_count') conditions.push("COALESCE(is_custom,0)=0 AND page_count IS NULL");
