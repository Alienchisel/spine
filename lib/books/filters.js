export const VIRTUAL_TAG_RULES = [
  {
    ...
    sql: "(year_edition IS NOT NULL AND (CAST(strftime('%Y','now','localtime') AS INTEGER) - year_edition) >= 100)",
  },
  {
    name: 'Vintage',
    test: (book) => {
    ...
    sql: "(year_edition IS NOT NULL AND (CAST(strftime('%Y','now','localtime') AS INTEGER) - year_edition) >= 50 AND (CAST(strftime('%Y','now','localtime') AS INTEGER) - year_edition) < 100)",
  },
  {
    name: 'Translated',
