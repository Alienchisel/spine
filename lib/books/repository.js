...
export function patchBook(id, patch) {
  ...
  const isFinishTransition = status === 'finished' && existing.status !== 'finished';
  // t() the supplied date so the dedup guard and the cascade insert below
  // compare normalized-to-normalized. effectiveDateStarted already runs
  // through t(); this removes the date_started/date_finished asymmetry so
  // findDuplicateRead's `date_finished IS ?` can't miss an already-logged
  // completion over a whitespace / exotic-hyphen difference and stack a
  // second reads row + double-bump read_count. The HTTP PATCH validates
  // the date shape first (isValidPartialDate rejects denormalized input
  // with 400), so this is defence-in-depth for in-process callers
  // (scripts / ingest) — matching the PUT and off-transition paths, which
  // already t().
