...
export function getBook(id) {
  ...
  const editions = book.work_id != null ? db.prepare(`
    SELECT id, title, format, status, date_finished, cover_path
    FROM books
...
export function listBooks(query) {
  ...
  // Layer 3 pass 3: per-book "currently reading" story, surfaced on the
  // Library Reading tab as a subline under the progress label. Only
  // populated for books with a story currently in status='reading'; the
  // first by position wins when (rare) multiple are simultaneously open.
  // Null when the book has no stories or none are 'reading'.
...
export function updateBook(id, payload) {
  ...
  // Defensive cover handling: distinguish "user explicitly cleared the cover"
  // (null/empty payload) from "user sent a malformed path that toFilename
  // can't normalize." The latter must preserve existing — silent destruction
  // is the bug class that nuked legacy .jpg covers when the regex was tightened.
  const newCoverFilename = toFilename(payload.cover_path);
  const userSentMalformed = payload.cover_path != null && payload.cover_path !== '' && newCoverFilename === null;
  const effectiveCoverPath = userSentMalformed ? existing.cover_path : newCoverFilename;
  const shouldDeleteOldFile = !userSentMalformed && existing.cover_path !== newCoverFilename;
  ...
  db.transaction(() => {
    ...
    // Edition propagation. When rating, review, or read_count change on a
    // book that's part of a linked-editions group (shared work_id), mirror
    // the change to its siblings so a 5-star rating on the digital edition
    // shows up on the physical and audio. Replaces an older heuristic that
    // matched by title + first-author name and fired on EVERY updateBook —
    // editing a description with `rating: null` in the spread payload would
    // silently clobber the rating on every book that happened to share a
    // title and first author. Gating on (a) explicit work_id link and (b)
    // actual change in those three fields keeps the user-facing intent
    // (linked editions stay in sync) without the false-positive clobber.
    const newRating = rating || null;
    const newReview = t(review);
    const ratingChanged    = newRating    !== (existing.rating ?? null);
    const reviewChanged    = newReview    !== (existing.review ?? null);
    const readCountChanged = newReadCount !== existing.read_count;
    if (existing.work_id != null && (ratingChanged || reviewChanged || readCountChanged)) {
      const setParts = [];
      const setVals  = [];
      if (ratingChanged)    { setParts.push('rating = ?');     setVals.push(newRating); }
      if (reviewChanged)    { setParts.push('review = ?');     setVals.push(newReview); }
      if (readCountChanged) { setParts.push('read_count = ?'); setVals.push(newReadCount); }
      setParts.push("updated_at = datetime('now', 'localtime')");
      db.prepare(`UPDATE books SET ${setParts.join(', ')} WHERE work_id = ? AND id != ?`)
        .run(...setVals, existing.work_id, id);
    }
  })();
