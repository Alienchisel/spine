...
function bookColumns(payload, extra = {}) {
  // Shelves only hold physical books. The unshelfed route and every shelf
  // drilldown filter `format = 'physical' OR format IS NULL`; the write
  // path mirrors that, so a direct API call can't store a non-physical
  // book at a shelf location the UI would never produce.
  const isShelvable = payload.format == null || payload.format === 'physical';
  ...
  return {
    ...
    source_type:        payload.fiction === false ? (t(payload.source_type) || null) : null,
    ...
    condition:          isPhysical ? (payload.condition || null) : null,
...
export function updateBook(id, payload) {
  ...
  db.transaction(() => {
    ...
    // Accept either name strings or {name} objects (the GET shape) — same
    // contract as syncPeople.
    const firstAuthorRaw = Array.isArray(authors) && authors.length > 0 ? authors[0] : null;
    const firstAuthor = typeof firstAuthorRaw === 'string'
      ? firstAuthorRaw.trim()
      : firstAuthorRaw?.name?.trim() || null;
