...
export function getBookFacets(query) {
  ...
  const lists       = db.prepare(`SELECT DISTINCT l.name FROM lists l JOIN list_books lb ON lb.list_id = l.id WHERE lb.book_id IN (SELECT id FROM books ${where}) ORDER BY l.name`).all(...params).map(r => r.name);
...
function bookColumns(payload, extra = {}) {
  ...
  return {
    ...
    room_id:            loc.room_id,
    unit_id:            loc.unit_id,
    // showcase_position is action-driven (via PATCH on the BookDetail page,
    // not via the form). POST/PUT pass it through if explicitly sent so
    // round-trips don't lose the slot; out-of-range or non-integer values
    // are coerced to null here as a final guard (validation rejects them
    // earlier with a clear message).
    showcase_position:  showcasePositionOrNull(payload.showcase_position),
  };
}

// 1..5 or null. Anything else (negative, zero, > 5, NaN, string '') becomes
// null so the column never carries a value that doesn't map to a slot.
function showcasePositionOrNull(v) {
...
export function createBook(payload) {
  ...
  // would create an inconsistent row.
  // Status side of the invariant: status='reading'/'finished' implies the
  // user has access to the book (owned or borrowed) — placeholder
  // semantic dissolves either way.
  const willBeOwned     = !!payload.is_custom || Number(payload.owned) === 1;
  const willBeActive    = payload.status === 'reading' || payload.status === 'finished';
  const effectiveIsStub = (payload.is_stub && !willBeOwned && !willBeActive) ? 1 : 0;
...
export function updateBook(id, payload) {
  ...
  // wishlist row stays a wishlist row).
  // Status side of the invariant: a status of reading/finished implies
  // the user has access to the book (owned or borrowed) — placeholder
  // semantic dissolves either way.
  const willBeOwned        = !!payload.is_custom || Number(owned) === 1;
  const willBeActive       = status === 'reading' || status === 'finished';
  const effectiveIsStub    = (is_stub && !willBeOwned && !willBeActive) ? 1 : 0;
...
export function patchBook(id, patch) {
  ...
  // is_stub / owned / status coupling: is_stub means "wishlist placeholder."
  // The invariant is that an active state and is_stub=1 cannot coexist.
  // Three cases force is_stub=0: the patch sets owned=1 (graduation on
  // acquisition); the patch moves status to 'reading'/'finished' (active
  // engagement implies access, owned or borrowed); or the patch tries to
  // set is_stub=1 while the existing row is already owned. Otherwise the
  // flag is user-controlled.
  const effectiveOwned   = owned !== undefined ? (owned ? 1 : 0) : existing.owned;
  const wouldBeOwned     = effectiveOwned === 1;
  const statusBecomesActive = status === 'reading' || status === 'finished';
  if (is_stub !== undefined || (owned !== undefined && wouldBeOwned) || statusBecomesActive) {
    fields.push('is_stub = ?');
    params.push(((wouldBeOwned || statusBecomesActive) ? 0 : (is_stub ? 1 : 0)));
  }
  ...
  // showcase_position is the rank (1–5) on the /showcase page. Null or
  // empty clears the slot. The client picks the next-empty slot for
  // additions and renumbers on reorder/remove, so the repo just round-trips
  // the value through the same normalisation as the POST path.
  if (showcase_position !== undefined) {
    fields.push('showcase_position = ?');
    params.push(showcasePositionOrNull(showcase_position));
  }
