...
// Column list for the list endpoint, computed once at module load from
// PRAGMA so future schema additions are picked up automatically. We drop
// `description` from the SELECT — at ~400 bytes/book it dominates the
// payload (≈25% of /api/books on a 200-row page), and no list-view
// component reads it (BookCard/Home/Diary/Readlist/ListDetail all only
// render summary fields; description is fetched again when the user
// lands on BookDetail). notes/review stay in because the Notes index
// page reads them from this endpoint, and they're typically NULL so
// the bandwidth cost is small.
const LIST_BOOK_SELECT = db.prepare('PRAGMA table_info(books)')
  .all()
  .map(r => r.name)
