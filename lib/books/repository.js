...
export function patchBook(id, patch) {
  ...
  // is_stub / owned coupling: is_stub means "wishlist placeholder."
  // The invariant is that owned=1 and is_stub=1 cannot coexist. Two
  // cases force is_stub=0: the patch sets owned=1 (graduation on
  // acquisition), or the patch tries to set is_stub=1 while the
  // existing row is already owned. Otherwise is_stub is user-controlled.
