...
import { t, normalizeIsbn, toFilename, toCoverUrl } from './normalization.js';
import { VIRTUAL_TAG_RULES, appendWhere, buildFilterConditions, buildOrderBy } from './filters.js';
import { syncAuthors, syncNarrators } from './people.js';
...
function bookColumns(payload, extra = {}) {
  const {
    title, status, owned, previously_owned, is_custom, is_stub, loved, fiction, source_type,
    cover_path, rating, date_started, date_finished, acquisition_source, acquisition_date,
    format, binding, condition, description, notes, review, page_count, duration_minutes,
    publisher, series, series_number, isbn_10, isbn_13, asin, language, original_language,
    translator, year_published, year_approximate, year_edition, shelf_id, building_id, room_id, unit_id,
  } = payload;
  return [
    t(title),
    status || 'unread',
...
const BOOK_INSERT_COLS = `title, status, owned, previously_owned, is_custom, is_stub, loved, fiction, source_type, cover_path, rating, date_started, date_finished, acquisition_source, acquisition_date, format, binding, condition, description, notes, review, page_count, duration_minutes, publisher, series, series_number, isbn_10, isbn_13, asin, language, original_language, translator, year_published, year_approximate, year_edition, shelf_id, building_id, room_id, unit_id`;
const BOOK_UPDATE_COLS = `title = ?, status = ?, owned = ?, previously_owned = ?, is_custom = ?, is_stub = ?, loved = ?, fiction = ?, source_type = ?, cover_path = ?, rating = ?, date_started = ?, date_finished = ?, acquisition_source = ?, acquisition_date = ?, format = ?, binding = ?, condition = ?, description = ?, notes = ?, review = ?, page_count = ?, duration_minutes = ?, publisher = ?, series = ?, series_number = ?, isbn_10 = ?, isbn_13 = ?, asin = ?, language = ?, original_language = ?, translator = ?, year_published = ?, year_approximate = ?, year_edition = ?, shelf_id = ?, building_id = ?, room_id = ?, unit_id = ?`;
const BOOK_PLACEHOLDERS = BOOK_INSERT_COLS.split(',').map(() => '?').join(', ');

export function createBook(payload) {
  const { authors, narrators, tags } = payload;
  const id = db.transaction(() => {
    const result = db.prepare(`INSERT INTO books (${BOOK_INSERT_COLS}) VALUES (${BOOK_PLACEHOLDERS})`).run(...bookColumns(payload));
...
export function updateBook(id, payload) {
  ...
  const { authors, narrators, tags, title, status, rating, review, is_stub } = payload;
  ...
  db.transaction(() => {
    db.prepare(`UPDATE books SET ${BOOK_UPDATE_COLS}, read_count = ?, updated_at = datetime('now') WHERE id = ?`)
      .run(...bookColumns(payload, { is_stub: effectiveIsStub }), newReadCount, id);
    ...
    if (t(title) && firstAuthor) {
      db.prepare(`
        UPDATE books SET rating = ?, review = ?, read_count = ?, updated_at = datetime('now')
        WHERE id != ? AND title = ?
