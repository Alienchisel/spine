...
export function getBook(id) {
  ...
  // per edition (MAX over reads with the book-column fallback during
  // the Phase 2 transition) so the EditionsSection badge reads
  // "Finished · 2025-06-12" when the user re-read the edition in 2025,
  // not the original first-read date the book column would carry.
  // Ordered by the same effective date so the sidebar lists editions
  // by recency-of-most-recent-engagement.
  const editions = book.work_id != null ? db.prepare(`
    SELECT b.id, b.title, b.format, b.status, b.cover_path, b.rating, b.read_count,
      COALESCE((SELECT MAX(date_finished) FROM reads WHERE reads.book_id = b.id), b.date_finished) AS date_finished
    FROM books b
