...
export function patchBook(id, patch) {
  const existing = db.prepare('SELECT id, current_page, current_minutes, cover_path, status, read_count, date_started, date_finished FROM books WHERE id = ?').get(id);
  if (!existing) return null;
  ...
  // is_stub / owned coupling: is_stub now means "wishlist placeholder."
  // Transitioning to owned=1 graduates the placeholder regardless of
  // what the client sent in is_stub, so wishlists never get stuck
  // marked-as-wishlist after acquisition. is_stub is otherwise
  // user-controlled.
  const graduating = owned !== undefined && Number(owned) === 1;
  if (is_stub !== undefined || graduating) {
    fields.push('is_stub = ?');
    params.push(graduating ? 0 : (is_stub ? 1 : 0));
  }
  ...
  if (notes               !== undefined) { fields.push('notes = ?');               params.push(t(notes)); }
  if (review              !== undefined) { fields.push('review = ?');              params.push(t(review)); }
