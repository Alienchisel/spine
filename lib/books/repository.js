...
// dates without status='finished'. The most recent reads row is the
// canonical target ("editing the latest read"); if none exists yet,
// a new row carries the supplied dates.
function syncLatestReadsRow(id, payload) {
  if (payload.date_started === undefined && payload.date_finished === undefined) return;
  const latest = db.prepare('SELECT id FROM reads WHERE book_id = ? ORDER BY id DESC LIMIT 1').get(id);
  if (latest) {
    const f = [];
    const p = [];
    if (payload.date_started  !== undefined) { f.push('date_started = ?');  p.push(t(payload.date_started)  || null); }
    if (payload.date_finished !== undefined) { f.push('date_finished = ?'); p.push(t(payload.date_finished) || null); }
    if (f.length) db.prepare(`UPDATE reads SET ${f.join(', ')} WHERE id = ?`).run(...p, latest.id);
  } else {
    db.prepare(`
      INSERT INTO reads (book_id, date_started, date_finished, created_at)
      VALUES (?, ?, ?, datetime('now', 'localtime'))
...
export function patchBook(id, patch) {
  ...
  db.transaction(() => {
    ...
    // Mirrors updateBook (PUT). date_started falls back to the latest
    // existing reads-row start (typically the in-progress reading
    // session that's being closed out) so a "mark finished" PATCH
    // after months of progress doesn't drop the start date already
    // recorded when reading began.
    if (isFinishTransition) {
      const existingStart = db.prepare(
        'SELECT date_started FROM reads WHERE book_id = ? ORDER BY id DESC LIMIT 1'
      ).get(id);
      db.prepare(`
        INSERT INTO reads (book_id, date_started, date_finished, created_at)
        VALUES (?, ?, ?, datetime('now', 'localtime'))
      `).run(
        id,
        (date_started !== undefined ? t(date_started) : existingStart?.date_started) ?? null,
