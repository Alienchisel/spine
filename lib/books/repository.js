...
export function patchBook(id, patch) {
  const existing = db.prepare('SELECT id, current_page, current_minutes, cover_path FROM books WHERE id = ?').get(id);
  if (!existing) return null;

  const { current_page, current_minutes, loved, on_readlist, is_stub, fiction, acquisition_source, description, archived, binding, format, condition, rating, publisher, isbn_10, isbn_13, acquisition_date, date_started, date_finished, year_published, year_edition, page_count, duration_minutes, series_number, authors, narrators, translators } = patch;
  ...
  if (date_finished       !== undefined) { fields.push('date_finished = ?');       params.push(date_finished || null); }
  ...
  db.transaction(() => {
    ...
    } else if (authors !== undefined || narrators !== undefined || translators !== undefined) {
