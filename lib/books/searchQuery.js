...
const SPECIAL_RE = /[\s()"]/;
// Bare article terms ("The Odyssey" → terms "the" + "odyssey") are dropped:
// libraries store titles both with and without leading articles, so a "the"
// atom AND-ed into the query would exclude the article-less variant — the
// exact miss that let The Odyssey exist in two work groups until the
// 2026-07-05 sweep. Quoted phrases and qualifier values are exempt (quoting
// is the deliberate-literal escape hatch); phrase matching instead gets an
// article-stripped title variant in atomSql/qatomSql below.
const ARTICLE_TERM_RE = /^(?:the|an|a)$/i;
...
export function tokenize(input) {
  ...
  while (i < input.length) {
    ...
    if (word === 'OR')      tokens.push({ type: 'OR' });
      ...
      if (stripped && !ARTICLE_TERM_RE.test(stripped)) tokens.push({ type: 'TERM', value: stripped });
    }
    i = j;
  }
  return tokens;
...
// "the odyssey" → "odyssey", so a phrase or qualifier literal that carries
// a leading article can still match the article-less stored title. Returns
// null when there's nothing to strip (or nothing would remain).
function articleStrippedVariant(normalized) {
  const s = normalized.replace(/^(?:the|an|a)\s+/, '');
  return s && s !== normalized ? s : null;
}

// Title LIKE clause that also tries the article-stripped variant of the
// literal, so `"The Odyssey"` finds the record titled "Odyssey". The
// stripped variant is prefix-anchored: it exists to catch the same title
// minus its article, not to relax a quoted phrase into a loose substring
// (`title:"The Republic"` should not match "Mortal Republic"). One
// direction suffices: the reverse case (stored "The Odyssey", query
// "Odyssey") already matches by substring.
function titleLike(normalized, like) {
  const stripped = articleStrippedVariant(normalized);
  return stripped
    ? { sql: '(nrm(title) LIKE ? OR nrm(title) LIKE ?)', params: [like, `${stripped}%`] }
    : { sql: 'nrm(title) LIKE ?', params: [like] };
}
...
function atomSql(literal) {
  ...
  const title = titleLike(normalized, like);
  const sql =
    `(${title.sql} OR nrm(COALESCE(series,'')) LIKE ?` +
  ...
  return { sql, params: [...title.params, like, like, like, like, like] };
}
...
function qatomSql(field, literal) {
  ...
  if (field === 'title') {
    const title = titleLike(normalized, like);
    return { sql: `(${title.sql})`, params: title.params };
  }
  let sql;
  switch (field) {
